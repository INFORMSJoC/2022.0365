function [aa,aax,bb,o,ytrain1,ytest,ratio,ratiov,ratiov1,I]=DBCD60block2fp27ggg(aa,aax,bb,x,y,C,B,p,lanf,c,qq,h,d)
n1=length(x(1,1,1,:,1));
n2=length(x(1,1,:,1,1));
n3=length(x(1,:,1,1,1));
n4=length(x(:,1,1,1,1));
    ...
    tt=ratiov1(I(ii),3);
            ...
            if(b1>sum(y2)&&b2>c(i,j,tt)*sum(y2))
                ...
                if(numel(a)==0)
                    ...
                    aax(i,j,:,tt)=permute(aax1(:,i,j,tt),[2 3 1 4]);
                    bb(i,j,:,tt)=permute(bb1(:,i,j,tt),[2 3 1 4]);
                    ytrain1(i,j,:,tt)=DBCD32bfpgg(aa,aax,bb,x,y,lanf,c,qq,tt,j,i,ytrain1);
